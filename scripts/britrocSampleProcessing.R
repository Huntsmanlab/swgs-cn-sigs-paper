...
deep_metrics <- read.csv("~/repos/cnsignatures/manuscript_Rmarkdown/data/britroc_deepWGS_ploidy_purity_49.csv", header=T, as.is = T)
deep_metrics$purity <- deep_metrics$purity/100
deep_metrics <- deep_metrics %>%
  dplyr::mutate(name= sub("[0-9]+_tumor_","",x=wgs_ID)) %>%
  dplyr::select(name,ploidy, purity) %>%
  tidyr::gather("metric", "deep", -name)

# Get ploidy for sWGS samples
CN <- readRDS(file = "~/repos/cnsignatures/manuscript_Rmarkdown/data/britroc_absolute_copynumber.rds")
Biobase::pData(CN)$ploidy <- GetPloidy(CN) %>%
  .$out

shallow_metrics <- Biobase::pData(CN) %>%
  dplyr::select(name, ploidy, purity) %>%
  dplyr::filter(name %in% deep_metrics$name) %>%
  tidyr::gather("metric", "shallow", -name)

# Annotate sWGS samples with star_rating
samp_annotation_all <- read.csv("~/repos/cnsignatures/manuscript_Rmarkdown/data/britroc_sample_data.csv", as.is=T)
samp_annotation <- samp_annotation_all %>%
  dplyr::filter(IM.JBLAB_ID %in% shallow_metrics$name & Failed !="Y") %>%
  dplyr::select(IM.JBLAB_ID , star_rating)

all_CN<-readRDS("~/repos/cnsignatures/manuscript_Rmarkdown/data/britroc_absolute_copynumber.rds")
